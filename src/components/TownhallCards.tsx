  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 p-8">
      {data.townhalls.map((townhall, index) => (
        <div
          key={index}
          className="bg-blue-200 text-center bg-opacity-10 backdrop-blur-md shadow-lg border border-opacity-20 rounded-lg  flex flex-col justify-between aspect-square overflow-hidden transform transition-transform duration-300 ease-in-out hover:scale-105"
        >
          <svg
          ...
          <h2 className="text-white  text-2xl font-semibold p-6 mb-8">
            {townhall.name}
          </h2>

          <p className="text-black text-lg font-bold p-6 text-opacity-80 mb-4">
